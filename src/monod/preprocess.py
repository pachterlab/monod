...
def threshold_by_expression(
    ...
    Parameters
    ----------
    mods: list of np.ndarrays
         [genes x cells modality 1, genes x cells modality 2, ...] (usually [unspliced, spliced] count matrices).
    ...
    for i in range(len(mods)):
        ...
        gene_exp_filter &= (mod_maxes[i] < filt_param['max_maxes'][i])
        gene_exp_filter &= (mod_maxes[i] > filt_param['min_maxes'][i])  
...
# This function seems already ready for arbitrary layer numbers.
