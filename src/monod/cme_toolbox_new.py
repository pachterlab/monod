...
class CMEModel:
    ...
    def eval_model_logL(self, p, limits, samp, data, n_cells, hist_type="none", EPS=1e-15):
        """Compute the negative log-likelihood of data under a set of parameters.
        ...
        if hist_type == "none":
            ...
            logL = np.log(pss[np.arange(len(data[0])), *data])
    ...
    def eval_model_kld(self, p, limits, samp, data, hist_type="none", EPS=1e-15):
    ...
    def eval_model_pss(self, p, limits, samp=None):
        ...
        # g is a 2d array with shape (n_species=len(limits),n_grids=mx!)
        for i in range(len(mx)):
            g[i] = g[i].flatten()
        ...
        samp_use = np.array([i for i in samp]+[0]*num_excess) # 1d array of sampling rates for all species
        ...
        if self.seq_model == "Poisson":
            ...
            if self.cell_size is None:
                ...
                cellwise_samp_use = samp_use[:, None] * self.cell_size[None,:]
                g = np.exp((np.power(10, cellwise_samp_use))[:, None] * g[:, None,:]) - 1
        ...
        if self.cell_size is None:
            ...
            new_shape = (len(cell_size),) + tuple(mx)
            gf = gf.reshape(new_shape)
    ...
    def protein_pgf(self, g, p):
        ...
        ----------
        g: np.ndarray
            complex PGF arguments, adjusted for sampling and subtracted by one.
