...
impl LibrarySearcher {
    ...
    pub fn finalize(self) -> Config {
        if let Some(ref inc_dir) = self.inc_dir {
            ...
            // the package updates.
            // Skip runtime version check when HDF5_NO_RUNTIME_CHECK=1 (e.g. cross-compilation).
            let skip_runtime_check = env::var("HDF5_NO_RUNTIME_CHECK").as_deref() == Ok("1");
            if !skip_runtime_check && !(self.pkg_conf_found && cfg!(windows)) {
